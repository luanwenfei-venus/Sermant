...
public class NettyGatewayClient implements GatewayClient {
    ...
    public void start() {
        ...
        nettyClient =
            NettyClientFactory.getInstance().getNettyClient(backendConfig.getNettyIp(), backendConfig.getNettyPort());
    }
    ...
    public boolean sendImmediately(Object object, int typeNum) {
        ...
        return dataType
            .filter(type -> nettyClient.sendInstantData(
                JSON.toJSONString(object, SerializerFeature.WriteMapNullValue).getBytes(StandardCharsets.UTF_8), type))
            .isPresent();
    }
